    ...
    if self.is_sponsor:
        ...
        if self.is_leader:
            ...
            for n in self.network_dict:
                ...
                new_ldr_msg.dict_data = {'type':'del_ldr','id':new_ldr_id,'ip': self.network_dict[new_ldr_id](0) ,'port':self.network_dict[new_ldr_id](1)}
                with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as soc:
                    soc.connect((new_ldr_msg.recv_host,new_ldr_msg._recv_port))
        ...
        delete_msg.dict_data = {'id':self.node_id}
