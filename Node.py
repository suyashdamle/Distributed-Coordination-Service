...
class Node(object):
	...
	def heartbeat_thread_fn(self):
		...
		"""
		respond to all heartbeats; if leader has sent a heartbeat, reset ldr_timeout_count to 0;
		else, increase it by one. When it reaches threshold, declare leader to be dead
		"""
		else:
			ldr_timeout_count = -1
			while True:
