...
class Node(object):
	...
	def __init__(self, config_fname=None,host=None,port=None):
		...
		self.PORT = 64531       		# Port to listen on
		...
		self.node_id = 2				# the node-id of this instance - changed during joining protocol
		...
		# Manual network-dict filling for now
		self.network_dict[1] = ('127.0.0.1',64532)
		...
		while inputs:
			...
			for s in readable:
				if s is server:
					...
					if msg:
						print("DEBUG_MSG: data received ",Msg_type(msg._m_type))
